...
class Model(nerf.Model):
    ...
    def build_networks(self, opt):
        ...
        torch.nn.init.normal_(self.graph.se3_refine.weight, mean=0, std=0.2)
    ...
    def setup_optimizer(self, opt):
        ...
        self.optim_pose = optimizer([dict(params=self.graph.se3_refine.parameters(), lr=opt.optim.lr_pose),
                                     dict(params=self.gnn.parameters(), lr=opt.optim.lr_gnn)])
        if opt.optim.sched_pose:
            ...
            self.sched_pose = scheduler(kwargs)
            
    def train_iteration(self, opt, var, loader):
        # return super().train_iteration(opt, var, loader)
